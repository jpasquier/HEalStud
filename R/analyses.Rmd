...
tbl_descr <- do.call(rbind, lapply(X, function(x) {
  ...
  names(tab) <- 
    c("Variable", "Type", "Nmiss", "Value", "(N)", "(Prop)")
  rownames(tab) <- NULL
  return(tab)
}))
rm(X)
...
## Effet modérateur de la variable MSPSS
...
m <- lm(WHOQOL_d2_raw_score  ~ PSS14_tot * GSES_tot + PSS14_tot * CD_RISC_tot +
          PSS14_tot * MAAS_tot + PSS14_tot * MSPSS_tot, hs)
summary(m)
