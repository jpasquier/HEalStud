...
subtitle: "`r if (!is.na(params$sttl)) params$sttl`"
...
temps <- params$temps
hs <- params$data[params$data$Temps == temps, ]
if (!is.na(params$dom)) {
  if (params$dom == "Soins infirmiers") {
    hs <- hs[to_factor(hs$Filiere) == params$dom, ]
  } else if (params$dom == "COVID19_4_Non") {
    hs <- hs[!is.na(hs$COVID19_4) & hs$COVID19_4 == 0, ]
  } else if (params$dom == "COVID19_4_Oui") {
    hs <- hs[!is.na(hs$COVID19_4) & hs$COVID19_4 == 1, ]
  } else if (params$dom == "Annee_1") {
    hs <- hs[!is.na(hs$Annee_etude) & hs$Annee_etude == 1, ]
  } else if (params$dom == "Annee_2") {
    hs <- hs[!is.na(hs$Annee_etude) & hs$Annee_etude == 2, ]
  } else if (params$dom == "Annee_3") {
    hs <- hs[!is.na(hs$Annee_etude) & hs$Annee_etude == 4, ]
  } else if (params$dom == "Annee_Autre") {
    hs <- hs[!is.na(hs$Annee_etude) & hs$Annee_etude == 0, ]
  } else if (params$dom == "Langue_FR") {
    hs <- hs[!is.na(hs$LANG_SAISIE) & hs$LANG_SAISIE == "FR", ]
  } else {
...
if (temps == 1) {
  X <- X[!(X %in% c("Bon", "Follow_up", "Recrutement"))]
  if (grepl("^COVID19_4", params$dom)) {
    X <- c(X, "COVID19_[1-3]", "PTGI_.+")
  } else {
    X <- c(X, "COVID19_[1-4]", "PTGI_.+")
  }
}
X <- paste0("^", X, "$")
...
tbl_descr <- do.call(rbind, lapply(X, function(x) {
  ...
  if (length(z) == 0) {
    return(NULL)
  } else if (any(class(hs[[x]]) %in% c("factor", "logical"))) {
...
<!--
(l'utilisation de la variable sexe exclut les répondants anglophones) :
-->
...
expand.grid(
  ...
  Sexe = names(attr(hs$Sexe, 'labels')[attr(hs$Sexe, 'labels') %in% hs$Sexe])
) %>%
  cbind(., pred_WHOQOL_d2_raw_score = predict(m, .)) %>%
  mutate(GSES_tot = factor(GSES_tot)) %>%
  ggplot(aes(x = PSS14_tot, y = pred_WHOQOL_d2_raw_score, color = GSES_tot)) +
...
do.call(rbind, lapply(grep("^PTGI", names(hs), value = TRUE), function(y) {
  if (length(unique(hs$COVID19_4)) == 1) {
    X <- "COVID19_3"
  } else {
    X <- paste0("COVID19_", 3:4)
  }
  do.call(rbind, lapply(X, function(x) {
    ...
    if (length(unique(na.omit(hs[[x]]))) <= 1) {
      pv <- NA
    } else {
